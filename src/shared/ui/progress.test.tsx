...
describe('<Progress/>', () => {
  ...
  it('renders correctly with progress 50', () => {
    const { getByTestId } = render(<Progress total={total} progress={50} />);
    expect(getByTestId('progress-bar-inner')).toHaveStyle({ width: '50%' });
  });

  it('renders correctly with progress 100', () => {
    const { getByTestId } = render(<Progress total={total} progress={100} />);
    expect(getByTestId('progress-bar-inner')).toHaveStyle({ width: '100%' });
  });

  it('updates width when progress prop changes', () => {
    const { rerender, getByTestId } = render(<Progress total={total} progress={50} />);
    expect(getByTestId('progress-bar-inner')).toHaveStyle({ width: '50%' });
    rerender(<Progress total={total} progress={75} />);
