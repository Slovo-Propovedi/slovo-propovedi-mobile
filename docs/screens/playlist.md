...
- Индикаторы скачивания: прогресс плейлиста, системные уведомления (`src/pages/playlist/lib/PlaylistOfflineNotifications.ts`, `notificationConstants.ts`). Итог скачивания: при полном успехе — «Добавлено в офлайн N проповедей»; при частичной неудаче — уведомление ошибки с текстом «Не удалось добавить в офлайн X из N» (неудачные треки остаются с иконкой «облако»); при потере сети посреди прогона — прогон прерывается, уведомление «Нет подключения к интернету» (трактуется как сетевая ошибка — без алерта `playlistCacheErrorAtom`). **Отменённый прогон не показывает уведомлений** (остановка через «Остановить добавление в офлайн» — тихая).
- Меню шапки (`PlaylistHeaderMenu`/`PlaylistHeaderMenuDropdown`, `src/pages/playlist/ui/`): пункты кэша — во время кеширования «Добавить все в офлайн» сменяется на **«Остановить добавление в офлайн»** (иконка `stop-circle-outline`), который живёт на месте «Добавить все в офлайн» и **всегда активен — включая офлайн** (отмена не требует сети). Подмена реактивная по атому `isCachingPlaylistAtom`. После остановки: активная закачка прерывается только если она принадлежит исключительно этому прогону (иностранные джойнеры manual/auto не задеваются), run-контроллер прерывается, оставшиеся записи очереди очищаются. Пункты истории (`usePlaylistHistoryMenu`, `PlaylistHistoryDialogs`): **«Пометить все прослушанными»** (иконка `checkmark-done`, виден когда есть playable-проповедь не завершённая) и **«Удалить проповеди из истории»** (иконка `trash-outline`, виден когда хотя бы одна проповедь плейлиста есть в истории) — оба с диалогом подтверждения; массовые экшены `markSermonsListenedAction`/`removeSermonsFromHistoryAction` (см. [features/listening-history.md](../features/listening-history.md) → «Массовые операции плейлиста»).
- Пункт **«Удалить из офлайн все»** дизейблится, когда `cachedCount === 0` **или** очередь непуста (реактивно по `cacheQueueAtom`) **или** есть идущая закачка (реактивно по `activeCacheUrlAtom`). Дополнительно press-time guard'ы `handleClearCacheOption`/`handleClearCacheConfirm` перепроверяют `hasInflightCacheDownloads()` (belt-and-suspenders на деструктивной очистке). От сети не зависит.
- **Глобальный стоп из развёрнутого плеера:** кеширование можно остановить не только из меню кэша плейлиста, но и из полноэкранного плеера — кнопка **«Остановить все закачки»** в правом верхнем углу (`HeaderOverlay` → `StopAllCachingButton` → `useStopAllCaching` → `cancelAllCacheDownloads`). Видна, когда очередь непуста или есть активная закачка; всегда активна, в т.ч. офлайн. Прерывает плейлист-прогон (тихо, без ложного «Добавлено в офлайн N из N»), отменяет активную закачку и дренирует очередь. Подробнее — [features/audio-cache.md](../features/audio-cache.md) → «Глобальный стоп».
- Статус-бар и цвет иконок шапки адаптируются к скроллу (`usePlaylistHeader`, `usePlaylistNavigationOptions`).

## Откуда данные

- Параметр маршрута `playlist` — **id плейлиста** (`useLocalSearchParams<{ playlist: string }>`). Резолвится в полный `PlaylistData` хуком `usePlaylistById` (`src/pages/playlist/lib/usePlaylistById.ts`) по трём источникам по порядку: 1) live `dynamicSectionsAtom`, 2) кэш (`resolvePlaylistFromCache` через `getCachedSections`, ключ `cachedSections`), 3) сетевой запрос `GET /playlists/{id}` (`resolvePlaylistFromApi` через сгенерированный orval-клиент, маппер `mapPlaylistEntityToPlaylistData`). Сеть вызывается **только** при промахе секций и кэша — благодаря этому deep link на плейлист открывается на «чистой» установке (cold start), когда секции ещё не загружены (их тянет экран «Слушать»). Пока идёт резолв — `isLoading`; после исчерпания всех источников без результата — `notFound` (сетевая ошибка тоже завершает резолв в `notFound`, без бесконечного спиннера). Состояние резолва сбрасывается при смене id плейлиста (warm-start навигация на другой плейлист).
...
- Тап на трек с `audioUrl` → `usePlayNewSermon({ playlist, sermon })` — запуск в плеере и открытие полноэкранного плеера. Повторный тап по тому же треку, пока запуск в полёте или в течение 1с после тапа, запустившего его, игнорируется (гард в `usePlayNewSermon`). Тап на уже играющий трек — no-op (Issue #99); тап на тот же трек на паузе возобновляет воспроизведение.
- «Слушать всё» → запуск первого трека с `audioUrl`.

## Состояния

- Загрузка: пока `usePlaylistById` резолвит плейлист из секций, кэша или сети (`isLoading`) — `PlaylistStatusView` (спиннер).
...
- Ошибка: плейлист не найден (нет в секциях, в кэше и по id в API) → `PlaylistStatusView` с `notFound` (заглушка); ошибки скачивания логируются и отображаются через диалоги/уведомления; одиночный трек ретраится (до 3 попыток, см. [features/audio-cache.md](../features/audio-cache.md)).
