...
- Офлайн: зависит от кэша треков (`cacheTrigger`); скачивание в офлайне недоступно — перед каждым треком проверяется подключение (`waitForOnline`, до 60с), при его отсутствии прогон прерывается уведомлением «Нет подключения к интернету».
