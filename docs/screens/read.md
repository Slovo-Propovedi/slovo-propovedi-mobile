...
**Статус:** **заблокирован** — кнопка в `CustomTabBar` (`src/widgets/tab-bar/ui/CustomTabBar.tsx`, `isDisabled={isUnavailableTabRoute(route.name)}`) показывает глобальный информационный диалог «Скоро будет доступно» (`showInfo`). UI частично готов.
...
- Таб доступен только через блокировку в `CustomTabBar`; нажатие показывает глобальный информационный диалог «Скоро будет доступно» (`showInfo`, текст «Этот раздел будет реализован в будущих обновлениях»).
