...
Размеры — именованные константы: `TOTAL_SIZE = 224` (габарит кнопки со свечением, **совпадает** с `glowConfig.RING_SIZE`; `ORBIT` и `BLOB_R` заданы долями от него — `0.27` / `0.19`), `INNER_SIZE = 168` (диаметр непрозрачного круга), `ICON_SIZE = 100`. **Layout-бокс кнопки** `block` — `width: '50%'` без внешних полей (ровно половина строки, как и секция справа в `FirstSectionRow`). Мягкое свечение выходит за границы `block`, но непрозрачный круг остаётся внутри, а правая секция не вылезает за экран. Pressed-состояние — `opacity: 0.8` при нажатии, disabled — `opacity: 0.5`.
