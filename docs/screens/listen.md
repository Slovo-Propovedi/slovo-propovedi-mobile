...
- **`Escape` в поиске (web-only)** — та же цепочка, что у «✕»: при непустом запросе очищает поле и оставляет поиск открытым (фокус сохраняется), при пустом — закрывает поиск. Работает **независимо от фокуса поля**: сфокусированный инпут — input-scoped `onKeyPress` (`handleKeyPress` с `isEscapeKey`), расфокусированный — document-bubble слушатель `useEscapeKey` (гейт `Platform.OS === 'web' && isSearchOpenAtom`); RNW-`stopPropagation` делает пути взаимоисключающими. На нативе не работает (гейт `Platform.OS === 'web'`). Слоистость с плеером и модалками — см. [features/web.md](../features/web.md) → «Клавиатура полноэкранного плеера (web)».
