...
**Safe-area:** `SafeAreaView` паддит только верх/лево/право (`edges={['top', 'left', 'right']}`) — низ не паддится, контент скроллится под полупрозрачный таб-бар и навбар-инсет; последний элемент очищается через `paddingBottom: tabBarHeight + PLAYER_SIZES.miniPlayerHeight` в обоих списках (`ScrollView` секций и `FlatList` результатов поиска).
...
- **История не пуста** → кнопка активна, play-иконка; возобновляет последнюю **незавершённую** проповедь (первая запись из `historyAtom`, у которой `getEntrySermon(entry)` определён и `isEntryCompleted(entry)` ложен; записи отсортированы по `lastPlayedAt` DESC, записи без проповеди и завершённые пропускаются). Выбор записи — через хук `useLastListeningEntry` (`entities/listening-history`).
...
- **`Escape` в поле поиска (web-only)** — та же цепочка, что у «✕»: при непустом запросе очищает поле и оставляет поиск открытым (фокус сохраняется), при пустом — закрывает поиск. Реализовано через `onKeyPress` на `TextInput` (хук `useSearchEscapeKey`, `src/features/sermon-search/lib/useSearchEscapeKey.ts`); на нативе не работает (гейт `Platform.OS === 'web'`). Слоистость с плеером и модалками — см. [features/web.md](../features/web.md) → «Клавиатура полноэкранного плеера (web)».
