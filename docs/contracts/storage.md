...
| `listeningHistory` | `LISTENING_HISTORY` — `src/shared/config/history-storage-keys.ts` | JSON `ListeningHistoryEntry[]` (Zod: `listeningHistorySchema`, safe-parse; невалидные данные → сброс в `[]`) | `writeHistory` — `src/entities/listening-history/lib/historyStorage.ts` (через `setCachedJson`) | `readHistory` — там же (через `getCachedJson` + `listeningHistorySchema`) | история прослушивания: массив (до 100 записей) снапшотов проповедей с позицией воспроизведения; записи **slim**: `{ playlist, positionMs, durationMs, lastPlayedAt }` — sermon живёт в `playlist.sermons[0]`, доступ через `getEntrySermon(entry)`; пишется **только по событиям** |
| `listeningProgressSnapshot` | `LISTENING_PROGRESS_SNAPSHOT` — там же | сырой JSON `{ sermonId, positionMs, durationMs }` (без zod; ручная валидация `isValidSnapshot`) | `writeLiveProgressSnapshot` — `src/entities/listening-history/lib/liveProgressStorage.ts` (каждый 5с-тик `usePlaybackProgressSaver`, **только** при воспроизведении) | `readLiveProgressSnapshot` / `reconcileOnHydration` — там же (при гидрации) | мини-снапшот текущего прогресса (~60 байт); при гидрации мержится в каталог (`durationMs = max`), осиротевший — дропается; чистится при flush/reconcile/remove/clear |
...
- **История прослушивания** (`listeningHistory`): пишется через `writeHistory` (сериализация через `setCachedJson` с Zod-валидацией `listeningHistorySchema`; невалидные данные → сброс в `[]`). Пишется **только по событиям** — НЕ по 5с-тикам: старт воспроизведения (`recordPlaybackStartAction` — позиция 0), flush при паузе (`flushHistoryProgressAction` — `PlaybackController.pause`, `WebPlayerService.pause`), переключение трека (`recordSermonSwitchAction` — из `usePlayNewSermon` и авто-перехода). Помечается как завершённая при окончании трека (`markHistoryCompletedAction` — `positionMs = durationMs`), вызывается из `TrackAutoAdvanceService.handleTrackEnd` ДО ветвления путей. Удаляется per-item (`removeHistoryEntryAction`) и полностью (`clearHistoryAction`) из экрана истории. Загружается при старте приложения (`loadHistoryAction` в `app/_layout.tsx`), при этом `reconcileOnHydration` мержит мини-снапшот `listeningProgressSnapshot` в каталог. Массив дедуплицируется по `getEntrySermon(entry).id` и обрезается до 100 записей (`sortAndCapEntries`). Логика — `src/entities/listening-history/`.
- **Мини-снапшот прогресса** (`listeningProgressSnapshot`): пишется через `writeLiveProgressSnapshot` каждый 5с-тик `usePlaybackProgressSaver` (сырой JSON ~60 байт, без zod) **только пока идёт воспроизведение** (с защитой skip-first-tick-после-переключения); каталог при этом не трогается. Читается при гидрации (`reconcileOnHydration`): мержится в совпадающую запись каталога (только если новее и запись не завершена), `durationMs = max(entry, snapshot)`; осиротевший снапшот дропается. Чистится при flush/remove/clear. Логика — `src/entities/listening-history/lib/liveProgressStorage.ts`, `reconcileOnHydration.ts`.
