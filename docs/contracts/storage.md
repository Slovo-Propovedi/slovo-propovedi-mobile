...
- **История прослушивания** (`listeningHistory`): пишется через `writeHistory` (сериализация через `setCachedJson` с Zod-валидацией `listeningHistorySchema`; невалидные данные → сброс в `[]`). Записи создаются при старте воспроизведения (`recordPlaybackStartAction` — позиция 0), обновляются раз в 5с (`usePlaybackProgressSaver` → `updateHistoryProgressAction`) и при паузе (`PlaybackController.pause`, `WebPlayerService.pause`). Помечается как завершённая при окончании трека (`markHistoryCompletedAction` — `positionMs = durationMs`), вызывается из `TrackAutoAdvanceService.handleTrackEnd` ДО ветвления путей. Удаляется per-item (`removeHistoryEntryAction`) и полностью (`clearHistoryAction`) из экрана истории. Загружается при старте приложения (`loadHistoryAction` в `app/_layout.tsx`). Массив дедуплицируется по `sermon.id` и обрезается до 100 записей (`sortAndCapEntries`). Логика — `src/entities/listening-history/`.
