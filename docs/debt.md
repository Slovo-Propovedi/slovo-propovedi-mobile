...
- [ ] **Воспроизведение из истории в исходном плейлисте** — сейчас тап из истории играет проповедь в снапшот-плейлисте из одной проповеди (entry.playlist); нужно хранить playlistId в записи истории и резолвить полный плейлист (очередь, авто-переход) — `src/pages/history/ui/HistoryRow.tsx`, `src/entities/listening-history/lib/buildHistoryEntry.ts`, `src/entities/listening-history/model/types.ts` — запрос пользователя, вернуться при следующей итерации истории.
