...
**Реэкспорты — только в корневом `index.ts` слайса.** Строки `export … from` живут только в корневом barrel-файле слайса (`index.ts`). Ни в каком другом исходном файле реэкспортов нет (исключения: `@x`-сегменты, точки входа `app/` для expo-router, платформенный barrel `entities/player/lib/PlayerService/index.ts`). Если файлу нужен символ из соседнего модуля — он импортируется напрямую (`import { X } from './cacheQueueState'`), а не реэкспортируется дальше. Это же правило закрывает сегментные barrel-файлы: `ui/index.ts`/`lib/index.ts` внутри слайса не существуют, а их бывшие экспорты либо поднимаются в корневой `index.ts` (если есть внешние потребители), либо остаются внутренними с прямыми относительными импортами.
