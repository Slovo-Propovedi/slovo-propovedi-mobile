...
- **Заголовок трека** (`MovingText`, `shared/ui/MovingText.tsx` — тонкая обёртка над `shared/ui/marquee-text/MarqueeText`) в мини-плеере, шапке полноэкранного плеера и строках треков: авто-скролл **стартует сам по себе** при геометрическом переполнении (`maxOffset > MARQUEE_EPSILON_PX`), без драг-гейта; между циклами пауза `MARQUEE_PAUSE` (2с). Драг **скрабит** заголовок (на нативе — `Gesture.Pan().activateAfterLongPress(250)`, на web — `minDistance(10)` + `failOffsetY([-14, 14])`); порог 3 px (`isRealDrag`) теперь только отличает реальный драг от медленного клика. **Все** тексты оцениваются геометрически — порога по длине строки нет (короткий, но широкий заголовок мариквится так же, как длинный). На web пост-драговый `click` глотается `useMarqueeClickGuard` (см. [web.md](./web.md) → «Marquee-текст»).
