...
| `PlaybackController.ts`                   | play/pause/stop/seek/setPlaybackRate/setVolume/getStatus; персист позиции; rate/volume делегирует в `playbackPreferences`                                                                                                                                   |
| `playbackPreferences.ts`                  | persisted rate/volume: хранит значения и ре-применяет их к свежему `AudioPlayer` (`applyPlaybackRate`/`applyVolume`, сводный `applyPreferences`), вызывается из `loadAudio`/`replaceAudio`                                                               |
...
| `webPlayerState.ts`, `webPlayerPubSub.ts` | состояние и pub-sub для веб-реализации. `webPlayerState` **зеркалит** `isPlaying`/`isBuffering`/`position` в общие Reatom-атомы (`setIsPlayingAction` и т.д.), которые читает UI (`usePlayerState`) — аналог status-листенеров expo-audio на нативе; без этого play/pause на web не реагировали                                                                                     |
| `webPlayerStubControls.ts`               | веб-заглушки методов без браузерного аналога (`getVolume`/`setVolume` — громкостью владеет браузер; `getStatus` — из web-стейта); домешиваются в `playerService` через `Object.assign`                                                                                                              |
