...
- **Заголовок трека** (`MovingText`, `shared/ui/MovingText.tsx`, react-native-text-ticker) в мини-плеере, шапке полноэкранного плеера и строках треков: заголовок **статичен** (`marqueeOnMount={false}`), авто-скролл стартует только после первого реального драга (перетаскивание долгим нажатием, `Gesture.Pan().activateAfterLongPress(250)`, порог `shouldArmMarquee` = 3 px — тот же паттерн, что у `MarqueeText`); при смене текста взвод сбрасывается (remount по `key`), новый заголовок снова статичен. **Все** тексты идут через один gated-ticker-путь — порога по длине строки нет (короткий, но широкий заголовок мариквится так же, как длинный); сам тикер инертен для помещающегося текста (`contentFits` в react-native-text-ticker не запускает анимацию). На web пост-драговый `click` глотается `useMarqueeClickGuard` (см. [web.md](./web.md) → «Marquee-текст»).
