...
| `webPlayerStubControls.ts`               | веб-заглушки методов без браузерного аналога (`setLockScreenMetadata`/`reassertLockScreenMetadata` — нет MediaSession; `getVolume`/`setVolume` — громкостью владеет браузер; `getStatus` — из web-стейта); домешиваются в `playerService` через `Object.assign`                                                                                                              |
