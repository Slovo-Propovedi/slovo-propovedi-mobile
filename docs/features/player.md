...
- **Marquee-компоненты мемоизированы (follow-up):** `MarqueeText` (`shared/ui/marquee-text`) и обёртка `MovingText` обёрнуты в `memo`; pan-жест строится через `useMemo` по стабильным shared values и `useCallback`-стабильному `startIdleMarquee`, а колбэк `useFrameCallback` — `useCallback([])`. Тики позиции аудио (~2/с) больше не пересоздают жест/RNGH-конфигурацию и не пере-регистрируют нативный frame-clock — корневая причина микро-статтера бегущей строки в полноэкранном плеере.
