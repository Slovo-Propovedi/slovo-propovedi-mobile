...
- `ui/PlaylistBottomSheet/` — шторка со списком треков плейлиста (`@gorhom/bottom-sheet`). Открытие — `showPlaylistAtom`. Шторка монтируется сразу на первом снапе (`FINAL_SNAP_INDEX = 0`, см. ниже), поэтому открытие — одна entrance-анимация без двойного перехода.
  ...
  **Снапы:** `['70%', windowHeight − topInset]` (геометрия — `ui/PlaylistBottomSheet/useQueueSheetSnapMetrics.ts`). `FINAL_SNAP_INDEX = 0` — шторка открывается на 70% (первый снап); свайп вверх по handle разворачивает на полную высоту минус статус-бар. При 70% снапе нижние `0.3·windowHeight − topInset` px списка обрезаются (clip edge). При полном снапе — clip edge = 0.

  **Гарантия скролла (Issue #69):** хук `useScrollGuarantee` (`ui/PlaylistBottomSheet/useScrollGuarantee.ts`) — динамический `ListFooterComponent` (spacer, `pointerEvents: none`). Высота футера = `max(84, frame − rawContent + 84 + hiddenBelowSheetEdge)`. При 70% снапе `hiddenBelowSheetEdge = 0.3·windowHeight − topInset` — последний ряд гарантированно очищает clip edge; при полном — 0, но фуутер обеспечивает физическую возможность начать drag (см. корневую причину Issue #69 ниже).

  **Прокрутка к текущей проповеди:** при открытии шторка **автоматически прокручивается к текущей проповеди** (она встаёт в верх списка, `viewPosition: 0`). Механизм — хук `useScrollToCurrentTrack` (`ui/PlaylistBottomSheet/useScrollToCurrentTrack.ts`). Скролл выполняется один раз за монт при оседании на первом снапе (`onChange(FINAL_SNAP_INDEX)`).
  ...
  **Фикс — инвентарь:**
  ...
  | Файл                                                           | Изменение                                                                                                                |
  | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
  | `useQueueSheetSnapMetrics.ts` (НОВЫЙ)                         | Геометрия снапов `['70%', H − topInset]`, `FINAL_SNAP_INDEX = 0`, `hiddenBelowSheetEdge` per-snap                         |
  | `useScrollGuarantee.ts` (НОВЫЙ)                               | Динамический footer-spacer (`ListFooterComponent`, `pointerEvents: none`), guarantee `max(84, frame − raw + 84 + hidden)` |
  | `useListReveal.ts`                                            | Порог `y > 0` (был `y > 100`); `IMMEDIATE_REVEAL_MAX_OFFSET_PX = 240`                                                   |
  | `PlaylistBottomSheet.tsx`                                     | `enableContentPanningGesture={false}`, `index={FINAL_SNAP_INDEX}`, snapPoints из хука                                    |
  | `model/useFullscreenPanGesture.ts`                            | `.enabled(!showPlaylist)` — пан-жест плеера выключен пока шторка открыта                                                  |
  | `PlaylistSheetList.tsx`                                       | `ListFooterComponent={footerSpacer}` из `useScrollGuarantee`                                                             |
