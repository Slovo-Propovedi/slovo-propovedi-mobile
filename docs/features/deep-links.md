...
- **dev-флейвор `ru.slovopropovedi.dev`** получает тот же `<intent-filter>` (манифест общий у обоих flavor) и **верифицируется так же, как prod**: его пакет добавлен вторым statement'ом в `assetlinks.json` (см. выше), поэтому `adb shell pm get-app-links ru.slovopropovedi.dev` тоже показывает `verified`. Нюанс: при **параллельной установке dev + prod** обе сборки заявляют один домен — система может показать выбор приложения (chooser) или открыть то, что назначено по умолчанию; назначить дефолт можно в системных настройках App Links (Settings → Open by default / «Открывать по умолчанию»).
