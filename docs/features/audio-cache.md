...
- **Методы:**
  ...
  - `cacheAudio(audioUrl, onProgress?, signal?)` — скачивание через `File.downloadFileAsync` (expo-file-system), `idempotent: true`, с ретраями и защитой от зависания (см. «Повторы и защита от зависания» ниже). Метод single-flight для всех вызывающих кодов: повторные вызовы с тем же URL возвращают общий промис (дублирующая скачка не запускается). При повторном вызове с `onProgress` — callback регистрируется в live-сете и получает текущий прогресс (retroactive seed) + все последующие тики через fan-out emitter (`inflightDownload.ts`). Третий аргумент `signal` (AbortSignal) — внешняя отмена (мост в `abortBridge.ts`). Файлы скачиваются во временное имя `<hash>.cache.mp3` (`PART_SUFFIX`); при успешном завершении файл атомарно переименовывается в `<hash>.mp3` (`File.rename()`). Частично скачанные файлы (`.cache.mp3`) никогда не распознаются как закэшированные (`getCachedUri`, `isCached` проверяют только финальный файл). При окончательной неудаче всех попыток `.cache.mp3`-файл **сохраняется** — это играбельный локальный источник для офлайн-воспроизведения скачанной части (см. «Офлайн-воспроизведение частично скачанного трека» ниже); при **отмене** `.cache.mp3`-файл удаляется немедленно (см. «Отмена скачивания»). Прогресс передаётся через `onProgress` callback expo-file-system (`{ bytesWritten, totalBytes }`), конвертируется в дробь 0..1 с троттлингом: тик эмитится только при дельте ≥ 0.01 **и** интервале ≥ `PROGRESS_TICK_MIN_INTERVAL_MS = 250` (`cacheDownloader.ts`); финальный тик при `fraction === 1` форсируется, прямые граничные вызовы `onProgress(0)`/`onProgress(1)` троттлинг обходят. Web-путь скачивания (`webAudioDownload.ts`) намеренно **не** троттлится — фикс Issue #100 мобильно-ориентированный, вне его скоупа. При отсутствии `Content-Length` (`totalBytes ≤ 0`) прогресс не обновляется (chunked transfer).
...
Когда приложение убивают посреди скачивания, в кэше могут остаться хвосты: на нативе — легаси-файлы `<hash>.mp3.part` (от версий приложения до переименования суффикса на `.cache.mp3`), на web — незакоммиченная запись в бакете. При следующем старте `cleanupOrphanedDownloads()` (`src/shared/lib/audio-cache/cleanupOrphans.ts` / `cleanupOrphans.web.ts`) чистит их — вызов цепочкой из `app/_layout.tsx` **до** `initializePlayer()`. Оба варианта **best-effort и никогда не бросают** (обёрнуты в try/catch → `console.error`) — обслуживание не должно ронять старт приложения.

- **Нативный sweep:** читает каталог кэша; если его нет — сразу выход; иначе удаляет каждый `File`, чьё имя оканчивается на легаси-суффикс `.mp3.part` (`LEGACY_PART_SUFFIX` в `cleanupOrphans.ts`) — такие файлы остались от версий приложения до переименования суффикса и не могут принадлежать активной закачке. Текущие частичные файлы `.cache.mp3` sweep **не трогает**: они намеренно сохраняются после неудачных закачек **и между сессиями** — это играбельный локальный источник для офлайн-воспроизведения скачанной части (см. «Офлайн-воспроизведение частично скачанного трека» ниже). Удаляется `.cache.mp3` только штатными путями — при **старте свежей закачки** того же URL (stale-drop в `downloadToCache`), при **удалении трека из офлайн** (`removeFromCache`) или при **полной очистке кэша** (`clearCache`). Заодно `getCacheInfo` (`AudioCacheService.ts`) теперь **исключает частичные файлы** — и `.cache.mp3`, и легаси `.mp3.part` — и из `fileCount`, и из `totalSize`: активные mid-download темпы не являются содержимым кэша.
...
- **Прогресс** — `onProgress(0)` эмитится в начале каждой попытки (UI сбрасывает полосу между попытками). Троттлинг тиков (≥1% дельты И ≥250мс) безопасен для stall-guard: `lastActivityAt` обновляется **сырыми** тиками до троттлинга, а таймаут 30с на порядок больше окна 250мс.
- **`.cache.mp3`-файл** — между попытками НЕ удаляется (`idempotent: true` перезапишет его); после окончательной неудачи всех попыток **сохраняется** (плюс `console.error`, затем ретроу последней ошибки) — это играбельный локальный источник для офлайн-воспроизведения скачанной части (см. «Офлайн-воспроизведение частично скачанного трека» ниже). Дополнительно `downloadToCache` при старте удаляет **осиротевший stale `.cache.mp3`** (остался от убитого приложения посреди скачивания) — чтобы он не занимал место и не мешал перекачиванию.
...
- `cancelAllCacheDownloads(ctx)` — **глобальный стоп** всех закачек (кнопка «Остановить все закачки» в правом верхнем углу полноэкранного плеера). Точный порядок: (0) если нет ни активной закачки, ни записей в очереди — ранний выход, стопперы не вызываются; (1) `invokePlaylistRunStopper()` — прерывает плейлист-прогон, чтобы он вернулся тихо (без ложного «Добавлено в офлайн N из N»); (2) если есть активная закачка (`activeCacheUrlAtom`) — `cancelCacheDownload(activeUrl)` (безусловно, даже с иностранными джойнерами); (3) дренаж: `removeFromQueue` для каждого URL в `cacheQueueAtom` (чистит requesters + прогресс + rejects deferred'ов `CacheCancelledError`). No-op при простое.
...
Отмена активной закачки через run-level stop («Остановить добавление в офлайн» в плейлисте) **не трогает иностранных джойнеров**: активная закачка прерывается только если **все** её requester'ы — `'playlist'` (`isOnlyPlaylistRequester` в `PlaylistOfflineService.ts`). Если к закачке присоединился `'manual'` или `'auto'` — run-level отмена его не заденет (ручной тап или авто-кэш воспроизведения не должны быть убиты остановкой плейлист-прогона).
...
- `registerPlaylistRunStopper(stopper)` / `unregisterPlaylistRunStopper(stopper)` — реестр в `cacheQueueState.ts`; `addPlaylistToOffline` регистрирует `() => controller.abort()` сразу после создания run-контроллера и снимает его в `finally`. Удаление **по идентичности** — `finally` старого прогона никогда не снимет стоппер нового.
...
2. `invokePlaylistRunStopper()` — прерывает run-контроллер плейлиста, цикл `runPlaylistCaching` выходит на ближайшем `if (signal.aborted) break`, и `addPlaylistToOffline` возвращается тихо через `if (controller.signal.aborted) return` — **без** ложного уведомления «Добавлено в офлайн N из N»;
...
- **Точечная подписка на скачивание per-row** — внутри `useTrackItemCache` (`src/shared/ui/track-list/useTrackItemCache.ts`): per-row `ctx.subscribe` на `playlistDownloadProgressAtom` с Object.is-bailout (паттерн `useIsCached`). Базовая строка `TracksListItem` сама выводит `isDownloading`/`isQueued`/`progressValue` из общих атомов очереди и прогресса; обёртки (`PlaylistTrackItem`, `PlaylistSheetRow`, `HistoryRow`, `OfflineRow`) больше **не** подписываются на скачивание и не передают boolean вниз. Списки (`PlaylistScreen`, `PlaylistBottomSheet`) не подписаны целиком — только совпадающий трек ре-рендерится при смене прогресса. Прежний хук `useIsDownloadingUrl` (`entities/player`, подписка на `downloadingAudioUrlAtom`) удалён как избыточный: `playlistDownloadProgressAtom` пишется тем же раннером очереди с тем же таймингом (первый progress-тик), а перепроверка кэша по завершении закачки теперь идёт через внутренний переход «качается → не качается» в `useTrackItemCache`.
...
- **`.cache.mp3`-файл удаляется на отмене** (натив): `throwIfCancelled` зовёт `deletePartFile(tempFile)` перед броском — не оставляет хвостов.
...
## Офлайн-воспроизведение частично скачанного трека (Issue #109 follow-up)

После обрыва сети посреди скачивания на диске остаётся частичный файл `<hash>.cache.mp3` (`PART_SUFFIX` = `.cache.mp3`, `src/shared/lib/audio-cache/cacheDownloader.ts`). Он **намеренно сохраняется** после окончательной неудачи всех попыток (раньше удалялся) — это играбельный локальный источник: iOS AVURLAsset типизирует локальные файлы по расширению пути, поэтому `.cache.mp3` распознаётся как аудио (в отличие от прежнего `.mp3.part`). Удаляется частичный файл при **явной отмене** закачки (`throwIfCancelled`), при **полной очистке кэша** (`clearCache` удаляет весь каталог), при **удалении трека из офлайн** (`removeFromCache` — best-effort чистка частичного файла рядом с финальным), при **старте свежей закачки** того же URL (stale-drop в `downloadToCache`) и при **онлайн-перезапросе аудио** (`deletePartialFile` в `resolvePlaybackUrl` — см. ниже).

Резолвер источника — `resolvePlaybackUrl` (`src/entities/player/lib/PlayerService/native/resolvePlaybackUrl.ts`, вызывается из `AudioLoader.loadAudio`/`replaceAudio`):

1. закэшированный финальный файл (`getCachedUri`) → играет `file://`;
2. иначе `startBackgroundCaching(audioUrl)` (fire-and-forget; офлайн попытка fast-fail'ится, retry-цикл держит частичный файл живым и пере-взводится на reconnect);
3. иначе частичный файл (`getPartialFileUri` из `partialFile.ts`) — **офлайн-фолбэк**: если пользователь офлайн (`!ctx.get(isOnlineAtom)`) → играет `file://` на скачанную часть; если онлайн → частичный файл **удаляется** (`deletePartialFile`) и резолвер переходит к сетевому URL — перезапрос аудио должен скачать полный файл, а не играть устаревшую часть;
4. иначе сетевой URL (стриминг).

`getPartialFileUri` (`src/shared/lib/audio-cache/partialFile.ts`) возвращает URI частичного файла только если он существует и непустой (`size > 0`); на web (нет файловой системы) try/catch безопасно резолвит `null`. Офлайн-guard (`guardOfflinePlayback`) разрешает воспроизведение трека с сохранённым частичным файлом без диалога на обоих нативных путях независимо от флага `allowPartiallyBuffered`; web резолвит `null` и сохраняет прежнее поведение. Слушатель может перематывать/слушать в пределах скачанной части. Если источник всё ещё сетевой (стрим завис офлайн), тап по прогрессбару подменяет источник на частичный файл и продолжает воспроизведение с выбранной позиции (см. [player.md](./player.md) → «Reconnect-heal»). При достижении края частичного файла воспроизведение **ставится на паузу**, а не переходит к следующему треку — авто-переход происходит только после полного прослушивания полной записи (см. [player.md](./player.md) → «Стриминг и кэш»); при исчерпании буфера стрима (частично пробуферизированная сессия) срабатывает штатная обработка столла.

## Автоматическое кэширование при воспроизведении

При старте трека `AudioLoader` через `resolvePlaybackUrl` (`src/entities/player/lib/PlayerService/native/resolvePlaybackUrl.ts`) сначала ищет кэш; если файла нет — плеер **стримит** с сервера (`downloadFirst: false`, HTTP range requests), а параллельно `startBackgroundCaching` (`BackgroundCachingService.ts`) ставит трек в глобальную очередь (`enqueueCache(ctx, url, 'auto', onProgress)`) для скачивания в офлайн-кэш. Дублирование скачивания одного URL блокируется самой очередью (dedup по queued/inflight). Глобальные атомы (`downloadingAudioUrlAtom`, `isDownloadingAtom`, `downloadProgressAtom`) пишутся **лениво** — только на первом progress-тике (`claimed`-флаг владельца): сама постановка в очередь НЕ «захватывает» downloader-состояние, пока URL ждёт своей очереди. Поздний stale-тик (после того как более новый downloader уже захватил состояние) не крадёт его обратно и не пишет прогресс (guard `ctx.get(downloadingAudioUrlAtom) === audioUrl` на каждую запись). При отмене — `console.warn('[BackgroundCaching] Caching cancelled:', …)`; сбой фонового кэширования обрабатывается молча (`console.error`), глобальный диалог ошибок НЕ показывается (Issue #73); воспроизведение не затрагивается (стриминг), а при следующем запуске того же трека кэширование ставится в очередь заново. Прежний модульный `Set` in-flight URL и механика «ADOPT» глобальных атомов (`_resetInFlightDownloadsForTesting` и т.п.) удалены — роль single-flight и прогресса теперь выполняет очередь + requester-реестр + lazy-запись.

> **Web (Issue #81):** авто-кэш при воспроизведении теперь работает **на обеих платформах**. На web хук — `autoCacheOnPlay` (`src/entities/player/lib/PlayerService/web/autoCache.ts`), вызывается из `WebPlayerService.loadAudio` (покрывает и `replaceAudio`): проверяет `audioCacheService.isCached`, при отсутствии в кэше и онлайне (`isOnlineAtom`) запускает `startBackgroundCaching`. Fire-and-forget, воспроизведение не блокирует. Нативный путь через `AudioLoader` → `resolvePlaybackUrl` — без изменений.
...
> **Возобновление после восстановления сети (Issue #109):** `setupReconnectRecovery` (`src/entities/player/lib/reconnectRecovery.ts`, вызывается первым в `initializePlayer` — до skip-restore early return) подписывается на `isOnlineAtom`; на переходе false→true `recoverAfterReconnect` пере-ставит `startBackgroundCaching` для текущего трека, если он ещё не закэширован, и запустит «лечение» стрима. Пере-постановка **пропускается**, когда URL уже стоит в serial-очереди или является активной закачкой (`isUrlQueuedOrActive` из `shared/lib/audio-cache`): такая закачка на нативе самовосстановится через retry-цикл (`waitForOnline` ждёт честного возврата интернета), а повторный `enqueueCache` лишь присоединился бы к умирающему промису и оставил трек «застрявшим» (Issue #109 follow-up). Если закачка оседает с ошибкой уже **после** reconnect-ребра (bounded retry-цикл исчерпан), `reconnectRecovery` пере-ставит её заново: опрос до освобождения URL (`restartCachingWhenSettled`, до 5 мин), затем повторный `startBackgroundCaching`. На web retry-цикла нет: уже начавшаяся закачка при сбое fast-fail'ится, а запись ждёт следующего reconnect-ребра или следующего воспроизведения, чтобы повториться (фоновое кэширование — невидимая оптимизация, семантика Issue #73). Лечение стрима при этом выполняется **независимо** от пропуска пере-постановки. Нативное лечение (`native/reconnectHeal.ts`): если источник — сетевой URL и плеер реально загружен (`AudioLoader.isPlayerLoaded`) — `replaceAudio` на сохранённой позиции (`positionAtom`); если плеер никогда не загружался (офлайн-старт, `waitForLoaded` таймаутнулся) — `loadAudio`; авто-resume только когда трек «завис» во время воспроизведения (`isPlaying && isBuffering`) — после паузы не возобновляем (pauseType ненадёжен). Web-лечение (`web/reconnectHeal.ts`): новый `HTMLAudioElement` на сохранённой позиции; новые слушатели `error`/`waiting`/`playing` делают web-состояние честным (error → isPlaying=false, waiting → isBuffering=true, playing → isBuffering=false). Офлайн-seek остаётся без диалогов: цель seek переживает в `positionAtom`, лечение возобновляет там же. Флапы сети не имеют cooldown'а by design: лечение перезагружает на той же позиции, очередь кэша дедуплицирует. Повторного сетевого запроса при уже закэшированном треке не происходит: на нативе `recoverAfterReconnect` пропускает `startBackgroundCaching`, если `audioCacheService.isCached` уже true, а `AudioLoader.loadAudio`/`replaceAudio` резолвят источник через `getPlaybackUrl` → `audioCacheService.getCachedUri` — при попадании в кэш плеер получает `file://` URI без единого сетевого запроса (`AudioLoader.ts:105-115`). На web `<audio>` использует сетевой URL, но `public/sw.js` обслуживает аудио cache-first через `serveCachedOrFetch` (`cache.match` → `serveWithRange`, иначе `fetch`) — закэшированный трек отдаётся из кэша, сеть не дёргается (`sw.js:162-165`).

> **Буферизованный прогресс офлайн (Issue #109 follow-up):** `BackgroundCachingService` дополнительно пишет «замороженный» объём буферизации в `bufferedProgressStateAtom` (`src/entities/player/lib/download-model.ts`, `{ progress, url }`) на **каждом** progress-тике (в том же guard `ctx.get(downloadingAudioUrlAtom) === audioUrl`). При успешном завершении закачки состояние очищается (`null`); при сбое/отмене — **сохраняется** (finally-блок его не трогает). Атом самонвалидирующийся: потребители сравнивают `url` с текущим треком, поэтому сброс при смене трека не нужен. Прогресс-бары (`MiniDownloadProgress`, `FullscreenDownloadProgressBar`) показывают `downloadProgress > 0 ? downloadProgress : bufferedProgress` — во время активной закачки живой прогресс, после обрыва сети — замороженный объём (не `Math.max`: перезапущенная закачка должна показывать свой реальный, возможно меньший, прогресс). Хук `useDisplayedDownloadProgress` (`src/widgets/expandable-player/model/useDisplayedDownloadProgress.ts`) резолвит состояние по URL и держит монотонную шкалу через `displayedProgressStateAtom` (`src/widgets/expandable-player/model/displayedProgressModel.ts`): при перезапуске закачки (восстановление сети) шкала не сбрасывается — она удерживает максимум показанного значения и продолжает расти, как только реальная закачка догоняет. Снапшот буфера игнорирует нулевые тики (каждая попытка закачки стартует с 0), поэтому шкала держит показания буфера во время обрыва сети.

> **Авто-resume после восстановления сети (Issue #109 follow-up):** `isStalledOfflineAtom` (`src/entities/player/lib/stalledOffline.ts`) — флаг «стрим завис офлайн». На нативе выставляется в `createAudioInterruptionHandler` (`nativePlayerHelpers.ts`) при прерывании, когда `isBuffering` (буферизационный столл сам по себе — признак голодания стрима; NetInfo-флип может отставать от underrun'а); на web — в `onError`, когда `isBuffering` (событие `waiting` приходит до сетевой ошибки; онлайн-сигнал так же может отставать от underrun'а, как и на нативе). Сбрасывается на загрузке нового источника (`AudioLoader.loadAudio`/`replaceAudio` — нативный chokepoint) и на web-событии `playing`. Лечение стрима (`reconnectHeal.ts`) резюмирует по `isPlaying || stalledOffline` в ветке, где плеер уже загружен (путь через `replaceAudio`) — трек, зависший офлайн, автоматически продолжает играть после возврата сети; в ветке «никогда не загружался» (зависание сразу после офлайн-старта) авто-resume по флагу stall не работает, возобновление следует только за `isPlaying`. Принятый trade-off: пауза **во время** офлайн-зависания тоже приведёт к авто-resume (флаг уже выставлен); пауза до зависания — нет.
...
- **Контекстное меню строки трека** (точки / долгое нажатие «Добавить в офлайн») — `TracksListItemContextMenu` → `useTrackItemCache.toggleCache` (`src/shared/ui/track-list/useTrackItemCache.ts`). В зависимости от состояния: качается/в очереди → `cancelCacheDownload`; закэширован → `removeFromCache`; облако → `enqueueCache(ctx, url, 'manual')` (гейт `isOnline`). После успеха (ветки добавить и удалить) инкрементит `cacheUpdateTriggerAtom`. Подробнее про `isQueued`-подписку и offline-поведение — в разделах ниже.
- **Меню полноэкранного плеера** `PlayerMenu` (`src/widgets/expandable-player/ui/PlayerMenu/PlayerMenu.tsx`) — пункт «Добавить в офлайн / Удалить из офлайн / Остановить добавление в офлайн / Убрать из очереди» → `useFullscreenHandlers.handleToggleCache` → `enqueueCache(ctx, url, 'manual')` / `cancelCacheDownload` / `removeFromCache` (перепроводка с прямого `cacheAudio` на очередь была частью Issue #83). После успеха (обе ветки) инкрементит `cacheUpdateTriggerAtom`.
...
## Офлайн: добавление в офлайн недоступно
...
- **Плейлист «Добавить все в офлайн»** — `usePlaylistOfflineMenu` (`src/pages/playlist/lib/usePlaylistOfflineMenu.ts`): `isAddAllToOfflineDisabled = allCached || !isOnline` (уже **не** включает `isCaching` — меню не блокируется во время кеширования; вместо «Добавить все в офлайн» показывается пункт «Остановить добавление в офлайн»). «Удалить из офлайн все» от сети не зависит.
- **Контекстное меню строки трека** — `useTrackItemCache` (`src/shared/ui/track-list/useTrackItemCache.ts`) подписывается на `isOnlineAtom` и возвращает `isCacheDisabled = !isOnline && !isCached && !isDownloading && !isQueued`: дизейблится **только** ветка «облако» (старт закачки офлайн); пункты «Остановить добавление в офлайн» (качается) и «Убрать из очереди» (в очереди) **всегда активны, в т.ч. офлайн** — отмена не требует сети. Поведенческий guard `toggleCache` гейтит сетью только cloud-ветку (`if (!isOnline) return` перед `enqueueCache`); cancel/remove работают офлайн. Ошибки ручного enqueue логируются `console.warn` (кроме `CacheCancelledError` — тихо).
- **Меню полноэкранного плеера** — `PlayerMenuItems` (`src/widgets/expandable-player/ui/PlayerMenu/PlayerMenuItems.tsx`): `isCacheDisabled = !isOnline && !isCached && visualState === 'cloud'`; строка «Добавить в офлайн» рендерится как обычный disabled-пункт меню. «Удалить из офлайн» (закэширован), «Остановить добавление в офлайн» (качается) и «Убрать из очереди» (в очереди) остаются активными офлайн. Поведенческий no-op guard в `useFullscreenHandlers.handleToggleCache` гейтит сетью только cloud-ветку.
...
- показывает системные уведомления (`PlaylistOfflineNotifications.ts`): начало, прогресс «Добавлено в офлайн N из M», завершение «Добавлено в офлайн N проповедей» либо ошибка «Не удалось добавить в офлайн X из N» при частичной неудаче (группа `playlist-cache`, фиксированный ID); **отменённый прогон НЕ показывает уведомлений** (в `addPlaylistToOffline` после `runPlaylistCaching` стоит `if (controller.signal.aborted) return`); **звук (Issue #101):** прогресс-уведомления (начало/обновление) — **без звука**, звук только на завершении/ошибке. На Android это реализовано через два канала (`ensurePlaylistNotificationChannels` в `playlistChannelIds.ts`): тихий канал `playlist-cache-progress` (`sound: null`) для прогресса и канал `playlist-cache-alert` (`sound: 'default'` — системный звук по умолчанию) для завершения/ошибки; `scheduleNotification` получает `channelId` и на Android шлёт `trigger: { type: 'channel', channelId }`, на iOS каналы игнорируются — там звук управляется контентом (`sound: false` для прогресса — `null` на iOS означал бы системный звук, `sound: true` для завершения/ошибки);
...
- `usePlaylistOfflineMenu.ts` — состояние меню кэша на экране плейлиста (диалоги подтверждения, позиция меню).
...
- `PlaylistHeaderMenu.tsx`, `PlaylistOfflineMenuItem.tsx`, `PlaylistHeaderMenuDropdown.tsx`, `PlaylistOfflineDialogs.tsx` — в `src/pages/playlist/ui/`.
...
- **Удаление:** `markUrlEvicted(ctx, url)` — из веток `removeFromCache` (`useTrackItemCache`, `useFullscreenHandlers`); `clearCachedUrls(ctx)` — из полных очисток кэша (`usePlaylistOfflineMenu.handleClearCacheConfirm`, `clearAudioCacheAction` в офлайн-меню). Оба заодно чистят персистентный реестр офлайн-проповедей (см. ниже).
...
## Реестр офлайн-проповедей (persistent offline registry)

`offlineRegistryAtom` (`src/shared/lib/audio-cache/offlineSermonsRegistry.ts`) — **персистентный** реестр URL закэшированного аудио → метаданные проповеди и плейлиста (`Record<url, { sermon: SermonData, playlist: PlaylistData | null, registeredAt: number }>`). В отличие от сессионного `cachedUrlsAtom`, переживает рестарты и не зависит от наличия метаданных в истории/секциях/поиске — это авторитетный источник экрана «Офлайн» (закрывает проблему «осиротевших» файлов кэша без метаданных и пустого web-списка).

- **Хранение:** AsyncStorage `offlineSermonsRegistry` (ключ `OFFLINE_SERMONS_REGISTRY`, `src/shared/config/cache-storage-keys.ts`), Zod `offlineSermonsRegistrySchema` при чтении. Записи сериализуются (promise-цепочка) — последняя запись всегда побеждает, без interleaved stale-записей.
- **Запись:** `registerOfflineSermon(ctx, url, sermon, playlist)` — синхронный reatom-action: no-change bailout (сравнение `JSON.stringify` метаданных, `registeredAt` исключён), иначе обновляет атом и fire-and-forget пишет в AsyncStorage (`console.warn` при сбое).
- **Гидрация:** `hydrateOfflineRegistry(ctx)` — при старте приложения (`useOfflineRegistrySync` в `app/_RootLayout.tsx`); невалидные/отсутствующие данные → пустой реестр.
- **Одноразовый бэкфилл:** `seedOfflineSermons(ctx)` (`src/features/offline-sermons/lib/seedOfflineSermons.ts`) — заполняет реестр из легаси-источников (история/секции/поиск/текущий плейлист) один раз; флаг `offlineSermonsRegistrySeeded` пишется только после успешного бэкфилла (сбой → повтор на следующем старте).
- **Синхронизация завершений:** `useOfflineRegistrySync` подписан на `cachedUrlsAtom` (narrow `ctx.subscribe`); новый URL (завершённое скачивание) → `syncOfflineSermon(ctx, url)` — проверяет `isCached`, находит проповедь в кандидатах (`collectSermonItems`) и регистрирует.
- **Удаление:** `markUrlEvicted` → `removeOfflineSermon(ctx, url)` (ветки `removeFromCache`); `clearCachedUrls` → `clearOfflineRegistry(ctx)` (полные очистки кэша). Оба no-op при отсутствии записи (ссылка атома не меняется).
- **Потребитель:** экран «Офлайн» — `collectOfflineRegistryCandidates` (`src/features/offline-sermons/lib/offlineRegistryCandidates.ts`) добавляет записи реестра в кандидаты мержа (порядок `[currentPlayer, sections, history, registry, search]`: свежие полные плейлисты бьют реестр, реестр бьёт синтетические плейлисты поиска).

## Очистка кэша

В меню шапки экрана «Офлайн» (`src/pages/offline/ui/OfflineHeaderMenu.tsx`) пункт «Очистить офлайн» → `ConfirmDialog` → `clearAudioCacheAction` (`src/shared/lib/audio-cache/clearAudioCacheAction.ts`) → `audioCacheService.clearCache()`. После очистки action сбрасывает optimistic-реестр (`clearCachedUrls`) и инкрементирует `cacheUpdateTriggerAtom` (строки/плейлист перепроверяют состояние). Пункт реактивно дизейблится, пока очередь непуста или идёт активная закачка (`cacheQueueAtom` + `activeCacheUrlAtom`); `clearAudioCacheAction` дополнительно перепроверяет `hasInflightCacheDownloads()` на нажатии — press-time guard, defense-in-depth (паритет с меню плейлиста).
...
- `offlineRegistryAtom` — персистентный реестр офлайн-проповедей (`Record<url, { sermon, playlist, registeredAt }>`), см. «Реестр офлайн-проповедей» выше;
- `playlistDownloadProgressAtom` — прогресс по URL (`Record<string, number>`). Запись идёт **только** через helper `cacheAudioWithProgress` (`shared/lib/audio-cache/cacheAudioWithProgress.ts`), который вызывается **раннером глобальной очереди** (`cacheQueueRunner.ts`) для **всех** источников (`'auto'`, `'manual'`, `'playlist'`). Helper пишет через `setTrackDownloadProgress` и чистит через `removeTrackDownloadProgress` (оба из `shared/lib/cache-triggers`) — запись создаётся до старта скачивания, тики обновляют прогресс, запись удаляется в `finally` (успех и ошибка); на успехе перед удалением прогресса вызывается `markUrlCached`. Глобального сброса атома в `{}` больше нет — его убрали из `PlaylistOfflineService.addPlaylistToOffline`, т.к. он стирал прогресс параллельных ручных скачиваний (Issue #82). Запись Record-per-tick (новая ссылка атома на каждый тик) **намеренна** и уже сужена потребителями (`useTrackItemCache` — `ctx.subscribe` + Object.is-bailout) — не «чинить» её позже (Issue #100).
...
1. **Старт воспроизведения** → `AudioLoader` → `resolvePlaybackUrl`: нет в кэше → `startBackgroundCaching` → `enqueueCache('auto')` (авто-кэш без действия пользователя, в глобальную очередь).
...
3. **Скачивание плейлиста** → `PlaylistOfflineService.addPlaylistToOffline` (все треки в очередь, последовательный прогон, с прогрессом и системными уведомлениями).
4. **Очистка кэша** → меню экрана «Офлайн» → `ConfirmDialog` → `clearAudioCacheAction`.
...
- [screens/offline.md](../screens/offline.md) — экран «Офлайн» (список закэшированного аудио)
