...
- **Web — journal + точечная очистка:** в `webDownloadJournal.ts` хранится AsyncStorage-журнал `audio-cache/active-downloads` (JSON-массив URL с идущей загрузкой; чтение оборонительное — легаси/битый JSON → пустой массив + лог). `downloadAndStoreAudio` пишет URL в журнал (`addActiveDownload`) **непосредственно перед** `fetch` (не на skip-пути — там скачивания нет, орфана быть не может) и снимает (`removeActiveDownload`) в `finally`, покрывающем fetch+put+commit. Стартовый `cleanupOrphanedDownloads.web` читает журнал; если он пуст — возвращается **не трогая `caches` вовсе** (обычные запуски остаются нулевой стоимости); иначе для каждого URL делает best-effort `deleteAudioEntry` (каждый в try/catch + лог), затем `clearActiveDownloads`. **Намеренно не перечисляет бакет** — перечисление возможно-повреждённого iOS-бакета может зациклить PWA на краше (WebKit bugs 260962/277598).
