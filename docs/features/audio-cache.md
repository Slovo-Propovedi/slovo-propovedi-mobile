...
После обрыва сети посреди скачивания на диске остаётся частичный файл `<hash>.cache.mp3` (`PART_SUFFIX` = `.cache.mp3`, `src/shared/lib/audio-cache/cacheDownloader.ts`). Он **намеренно сохраняется** после окончательной неудачи всех попыток (раньше удалялся) — это играбельный локальный источник: iOS AVURLAsset типизирует локальные файлы по расширению пути, поэтому `.cache.mp3` распознаётся как аудио (в отличие от прежнего `.mp3.part`). Удаляется частичный файл только при **явной отмене** закачки (`throwIfCancelled`), при **полной очистке кэша** (`clearCache` удаляет весь каталог), при **удалении трека из офлайн** (`removeFromCache` — best-effort чистка частичного файла рядом с финальным) и при **старте свежей закачки** того же URL (stale-drop в `downloadToCache`).
...
3. иначе частичный файл (`getPartialFileUri` из `partialFile.ts`) → играет `file://` на скачанную часть;
