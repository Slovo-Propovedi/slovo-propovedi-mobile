...
- **Без перечисления бакета вне commit-пути:** ни один read/has/delete/cleanup-путь не вызывает `cache.keys()`. `hasCompleteAudio` читает манифест read-only (`readCommittedUrls`); при отсутствии манифеста — match-only фолбэк (легаси-доверие) без построения. `uncommitAudioUrl` при отсутствии манифеста — no-op (нет манифеста ⇒ нечего отзывать). Перечисление бакета существует только внутри commit-пути (`commitAudioUrl` → `ensureManifest` → `buildManifest`) как одноразовая легаси-миграция на первом коммите.
