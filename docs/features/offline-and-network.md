...
- `isInternetReachable === null` (в первом эмите — `undefined`) — значение ещё неизвестно: transient-окно до первого reachability-проба. На web NetInfo **резолвит** reachability: JS-слой после mount'а делает пробу и пере-эмитит boolean (первое состояние — `undefined`, не `null`). В этом окне фолбэк на `isConnected` сохраняет прежнее поведение: web и холодный старт считаются онлайн по интерфейсу.
...
- **Записанное решение (false-negative trade-off):** если сеть блокирует reachability-пробу (captive portals, некоторые корпоративные/VPN/цензурирующие сети), `isInternetReachable=false` при реально работающем трафике — приложение тогда показывает `NetworkBanner`, блокирует незакэшированное воспроизведение, дизейблит «добавить в офлайн» и пропускает проверки обновлений. Это осознанно: прежнее поведение на `isConnected` — баг #109.
...
- **офлайн + трек не закэширован, но частично пробуферизирован в этой сессии** (toggle-play и web media-session) → воспроизведение разрешено: слушатель дослушивает буферизованную часть, при исчерпании буфера срабатывает штатная обработка столла;
...
2. **Toggle-play (пауза → play)** — единый хук `useGuardedTogglePlay` (`src/entities/player/lib/useGuardedTogglePlay.ts`): при блокировке `play()` не вызывается, плеер остаётся на паузе. Пауза (play → pause) guard'ом не блокируется. При разрешённом resume `resumeAfterPause` меняет источник на закэшированный `file://`, когда он доступен (Issue #107) — это же лечит офлайн+cached resume от устаревшего серверного URI. Хук используется всеми play/pause-кнопками: полноэкранный плеер (`useFullscreenHandlers`), мини-плеер (`ExpandablePlayer`) и кнопка play в `PlayerControls`.
...
- **Offline:** `NetInfo` → `isOnlineAtom = false` → показывается `NetworkBanner`; API-вызовы падают с сетевой ошибкой → `reportServerUnreachable` → `ServerErrorToast`; `fetchAllSections` показывает кэш секций; поиск проповедей показывает per-query кэш (`cachedSermonSearch:<query>`), если он есть; подсказки поиска берутся из кэша `cachedDistinctValues`, если он есть (иначе скрываются); аудио играет из кэша, а **частично скачанный трек** (сохранённый `.cache.mp3` после обрыва закачки) тоже играбелен офлайн — слушатель слушает/перематывает скачанную часть (см. [audio-cache.md](./audio-cache.md) → «Офлайн-воспроизведение частично скачанного трека»). Если стрим оборвался посреди закачки, прогресс-бары плеера продолжают показывать замороженный объём буферизации (`bufferedProgressStateAtom`, Issue #109 follow-up), а зависший офлайн трек автоматически возобновится при возврате сети (`isStalledOfflineAtom`).
- **Online:** `isOnlineAtom = true` → баннер скрывается; `useOfflineRetry` немедленно перезапрашивает данные; успешные ответы → `reportServerReachable`; поиск пишет свежие результаты в кэш; `reconnectRecovery` (Issue #109) пере-ставит фоновое кэширование текущего трека и «вылечит» стрим — пере-постановка переживает закачку, которая осела с ошибкой уже после reconnect-ребра (см. [audio-cache.md](./audio-cache.md) → «Возобновление после восстановления сети»).
