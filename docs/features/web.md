...
- `web/interruptionResume.ts` (+ `web/visibilityWatcher.ts`) — восстановление позиции после iOS-прерывания (Issue #106): снапшот позиции (`noteLivePosition`/`noteExplicitPosition`/`reset`, guard 1с), `maybeRestore` из `play()` и visibility-колбэка, одноразовый `loadedmetadata`-ретрай при `readyState === 0`, flush `max(currentTime, snapshot)`; `web/mediaSession.reassert()` перерегистрирует media-session-хендлеры при возврате на вкладку. Подробнее — [player.md](./player.md) → «Web-прерывание: восстановление позиции (Issue #106)».
