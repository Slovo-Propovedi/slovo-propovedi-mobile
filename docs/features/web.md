...
`shared/ui/marquee-text` — авто-скролл текста при переполнении + перетаскивание долгим нажатием (используется в `SliderItemDescription`; `PlaylistListItem` использует обычный многострочный `Text`). Авто-скролл **стартует сам по себе** при геометрическом переполнении: как только измерение даёт `maxOffset > MARQUEE_EPSILON_PX`, `useAnimatedReaction` вызывает `startIdleMarquee` — цикл запускается сразу после измерения, без ожидания драга; между циклами пауза `MARQUEE_PAUSE` (2с). При смене текста (новый заголовок) анимация сбрасывается (`translateX = 0` в `useEffect` по `text`), и новый заголовок заново оценивается на переполнение. Драг **скрабит** текст (`translateX` следует за пальцем/курсором с clamp по `maxOffset`), а по завершении реального драга цикл перезапускается. Порог `DRAG_ACTIVATION_THRESHOLD_PX` (3 px, предикат `isRealDrag(e.translationX)` в `createMarqueeGesture.ts`) больше **не гейтит** marquee — он только отличает реальный драг от медленного клика: суб-пороговый жест сбрасывает `didDrag`, чтобы клик не глотался и навигация работала. `shared/ui/MovingText` — тонкая обёртка над `MarqueeText` (`shared/ui/marquee-text/marquee-text`): прокидывает `text`/`testID`, окрашивает `textStyle` цветом темы (`currentTheme.text`) и добавляет внешний `style`; заголовки мини-плеера/полноэкранного плеера/строк треков скроллятся тем же авто-скроллом на переполнении (`centerWhenStatic` по умолчанию false — выравнивание влево). Право на marquee — **только геометрическое переполнение** (`maxOffset > MARQUEE_EPSILON_PX`: измеренная ширина текста больше контейнера более чем на эпсилон 1 px), порога по длине строки нет: `shouldMarquee(maxOffset)` у `MarqueeText` не смотрит на число символов, а `MovingText` делегирует всю геометрию `MarqueeText` — дубликат со спейсером рендерится только при переполнении, помещающийся текст остаётся одной статичной копией. Это чинит баг «короткий, но широкий заголовок» (мало символов, широкие глифы): раньше такой текст попадал в статичную ветку по порогу длины и висел с многоточием навсегда. На web работает через те же Reanimated-ворклеты (на web worklets исполняются на JS-потоке, `scheduleOnUI` → `requestAnimationFrame`), но есть пять платформенных особенностей:
