...
- **`Escape` поверх модалки** — закрывает верхнюю модалку (`shared/ui/modal.tsx` через `useEscapeKey` из `shared/lib/escape-key`, web-only слушатель `keydown` на `document` в capture-фазе). Слоистость: при открытой модалке (например, выбор темы) Esc закрывает **только её** — `stopPropagation()` не даёт событию дойти до window-слушателя плеера (`usePlayerKeyboardSeek`), который иначе свернул бы плеер. Модификаторы (ctrl/meta/alt/shift) не перехватываются. На нативе модалку закрывает hardware-back через `onRequestClose` — изменений нет. Закрывается **одна** модалка за нажатие: у каждого смонтированного `shared/ui/modal` свой capture-слушатель, поэтому два одновременно открытых инстанса закроются оба; сегодня единственный потребитель — `ThemeDialog`, коллизии нет.
- **`Escape` в поиске** — `SearchBar` (`features/sermon-search`, web-only, предикат `isEscapeKey` из `shared/lib/escape-key`): при непустом запросе очищает поле и **оставляет поиск открытым** (фокус сохраняется); при пустом запросе закрывает поиск целиком (та же цепочка, что у кнопки «✕»). Работает **и с фокусом на поле, и без него**: сфокусированный инпут обрабатывает input-scoped `onKeyPress` (`handleKeyPress` с `isEscapeKey`); расфокусированный — событие всплывает до `document`, где его ловит `useEscapeKey` из `shared/lib/escape-key` (bubble-слушатель, гейт `Platform.OS === 'web' && isSearchOpenAtom`). Пути взаимоисключающие: RNW-`TextInput` вызывает `stopPropagation()` в своём `handleKeyDown` до пользовательского `onKeyPress`, поэтому событие либо умирает на инпуте (фокус), либо доходит до document (без фокуса) — двойного срабатывания нет. Событие не доходит до window-слушателя плеера: в фокусе его гасит RNW-инпут, на document-пути — `stopPropagation()` из `useEscapeKey`, плюс плеер пропускает editable-таргеты (`isEditableTarget`). Модалка по-прежнему выигрывает: её capture-слушатель на `document` срабатывает раньше и `stopPropagation()` не даёт событию дойти до инпута. Модификаторы (ctrl/meta/alt/shift) не перехватываются. На нативе поведение не меняется (гейт `Platform.OS === 'web'`).
