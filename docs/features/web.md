...
- `webPlayerStubControls.ts` — веб-заглушки методов без браузерного аналога: `getVolume` / `setVolume` (громкостью владеет браузер), `getStatus` (собирается из web-стейта). Домешиваются в `playerService` через `Object.assign`.
...
- **`Escape`** — сворачивание плеера (`collapsePlayer`): если открыта шторка плейлиста — сначала закрывает её, иначе сворачивает плеер (та же цепочка, что у кнопки-шеврона). Обрабатывается слоем в общем стеке Escape (см. ниже); `preventDefault()` — чтобы браузер не выходил из нативного fullscreen.
...
`shared/ui/marquee-text` — авто-скролл текста при переполнении + перетаскивание долгим нажатием (используется в `PlaylistListItem` и `SliderItemDescription`). Авто-скролл **не стартует сам по себе**: заголовок статичен (`translateX = 0`), пока пользователь не перетащит его вручную (на нативе — палец с long-press, на web — LMB-press-drag) хотя бы на `DRAG_ACTIVATION_THRESHOLD_PX` (3 px). После первого реального драга marquee «взводится» (`marqueeArmed` shared value в `useMarqueeAnimation.ts`, ставится в `onEnd` жеста по предикату `shouldArmMarquee(e.translationX)`) и цикл авто-скролла запускается — дальше он крутится как раньше, с паузой `MARQUEE_PAUSE` (2с) между циклами; при смене текста (новый заголовок) взвод сбрасывается, и новый заголовок снова статичен до первого драга. Тот же принцип применён к `shared/ui/MovingText` (react-native-text-ticker, заголовки мини-плеера/полноэкранного плеера/строк треков): заголовок статичен (`marqueeOnMount={false}`), авто-скролл стартует только после первого реального драга (тот же `Gesture.Pan().activateAfterLongPress(250)` + `shouldArmMarquee`), при смене текста сбрасывается (remount по `key`). Право на marquee — **только геометрическое переполнение** (`maxOffset > MARQUEE_EPSILON_PX`: измеренная ширина текста больше контейнера более чем на эпсилон 1 px), порога по длине строки нет: `shouldMarquee(maxOffset)` у `MarqueeText` не смотрит на число символов, а `MovingText` гонит **все** тексты через один gated-ticker-путь — сам тикер инертен для помещающегося текста (`contentFits` в react-native-text-ticker не запускает анимацию и не рендерит дубликат со спейсером). Это чинит баг «короткий, но широкий заголовок» (мало символов, широкие глифы): раньше такой текст попадал в статичную ветку по порогу длины и висел с многоточием навсегда. На web работает через те же Reanimated-ворклеты (на web worklets исполняются на JS-потоке, `scheduleOnUI` → `requestAnimationFrame`), но есть пять платформенных особенностей:
...
- **Курсор `grabbing`** на время драга (восстанавливается после `pointerup`/`pointercancel`).
- **Клик после драга глотается.** Как только суммарное движение превысило 5 px, на обёртку вешается одноразовый capture-слушатель `click` с `preventDefault` + `stopPropagation` — иначе браузер после драга диспатчил бы `click` на карточку и навигировал. Клик без драга (≤ 5 px) не трогается — карточка открывается как обычно.
