...
3. **Каждый «срезанный угол»** (TODO, hack, отложенная задача, workaround) → запись в `docs/debt.md` в том же PR. Формат:
   ...
   `docs/debt.md` — только открытый бэклог: закрытая запись **удаляется** из файла в том же PR (вместо пометки `[x]`), краткое резюме закрытия — в body коммита, знание — в профильных `docs/features|contracts` (правило 2).
